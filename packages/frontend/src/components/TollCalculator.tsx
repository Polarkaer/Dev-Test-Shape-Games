...
export default function TollCalculator() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('http://localhost:4000/api/passages/daily-toll', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ vehicleType, passages: passageList.map(ts => ({ timestamp: ts })) }),
      });
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', maxWidth: '700px' }}>
      <div>
        <h2>Toll Fee Calculator</h2>
        <label style={{ display: 'block', marginBottom: '0.5rem' }}>Vehicle Type:</label>
        <select value={vehicleType} onChange={(e) => setVehicleType(e.target.value)} style={{ width: '100%', padding: '0.5rem' }}>
      ...
      <div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr auto', gap: '0.5rem', alignItems: 'end' }}>
          ...
          <Button type="button" onClick={addPassage} style={{ padding: '0.5rem 1rem' }} className='button'>Add</Button>
        </div>
      ...
      {passageList.length > 0 && (
        <div>
          ...
          <table style={{ borderCollapse: 'collapse', width: '100%', fontSize: '0.9rem' }}>
            <thead>
              <tr>
                <th style={{ border: '1px solid #ccc', padding: '0.5rem', textAlign: 'left' }}>Timestamp</th>
                <th style={{ border: '1px solid #ccc', padding: '0.5rem', width: '80px' }}>Action</th>
              </tr>
            </thead>
            <tbody>
              {passageList.map((ts, i) => (
                <tr key={i}>
                  <td style={{ border: '1px solid #ccc', padding: '0.5rem' }}>{ts}</td>
                  <td style={{ border: '1px solid #ccc', padding: '0.5rem', textAlign: 'center' }}>
                    <Button type="button" onClick={() => removePassage(i)} style={{ padding: '0.25rem 0.75rem', color: 'red' }}>Remove</Button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <Button 
        ...
        className='button'
        style={{ padding: '0.75rem', fontSize: '1rem', cursor: passageList.length === 0 ? 'not-allowed' : 'pointer' }}
      >
        {loading ? 'Calculating...' : 'Calculate Total Fee'}
      </Button>

      {result && (
        <div style={{ backgroundColor: '#000', padding: '1rem', borderRadius: '4px' }}>
          <h3>Result</h3>
          <p><strong>Date:</strong> {result.date}</p>
          <p><strong>Total Fee:</strong> <span style={{ fontSize: '1.5rem', color: 'green', fontWeight: 'bold' }}>{result.totalFeeDkk} DKK</span></p>
          {result.chargedPassages && result.chargedPassages.length > 0 && (
            <div style={{ marginTop: '1rem' }}>
              ...
              <table style={{ borderCollapse: 'collapse', width: '100%', fontSize: '0.85rem' }}>
                <thead>
                  <tr>
                    <th style={{ border: '1px solid #ccc', padding: '0.5rem', textAlign: 'left' }}>Window</th>
                    <th style={{ border: '1px solid #ccc', padding: '0.5rem', textAlign: 'right' }}>Fee (DKK)</th>
                    <th style={{ border: '1px solid #ccc', padding: '0.5rem' }}>Triggering</th>
                  </tr>
                </thead>
                <tbody>
                  {result.chargedPassages.map((cp, i) => (
                    <tr key={i}>
                      <td style={{ border: '1px solid #ccc', padding: '0.5rem' }}>
                        {new Date(cp.windowStart).toLocaleTimeString('en-DK')} - {new Date(cp.windowEnd).toLocaleTimeString('en-DK')}
                      </td>
                      <td style={{ border: '1px solid #ccc', padding: '0.5rem', textAlign: 'right' }}>{cp.appliedFeeDkk}</td>
                      <td style={{ border: '1px solid #ccc', padding: '0.5rem', fontSize: '0.8rem' }}>{cp.triggeringTimestamp}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
